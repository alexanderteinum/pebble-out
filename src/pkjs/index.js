function parseWeather(json) {
  ...
  for (var i = 0; i < limit; i++) {
    ...
    // 1. Find highest UV index in the 3-hour window
    var uv = data.instant.details.ultraviolet_index_clear_sky || 0;
    if (uv > maxUV) {
      maxUV = uv;
    }

    // 2. Get the current temperature from the first data point only
    if (i === 0 && data.instant.details.air_temperature !== undefined) {
      currentTemp = data.instant.details.air_temperature;
    }

    // 3. Calculate total probability for the 3-hour window
